...
if __name__ == "__main__":
    ...
    lpath = 'models\\haarcascade_eye.xml'
    rpath = 'models\\haarcascade_eye.xml'
    face_detector, left_eye_detector, right_eye_detector = load_face_eye_models(fpath, lpath, rpath)
    model = load_blink_model( "models\\eb.h5")
            ...
            if ret:
                ...
                if len(faces)>0:
                    ...
                    for (ex,ey,ew,eh) in left_eye_pos:
                        color = (0,255,0)
                        eye_img = cv2.resize(gray_face[ey:ey+eh,ex:ex+ew], dsize=IMG_SIZE)
                    ...
                    #     eye_img_l = gray_face[ey:ey+eh,ex:ex+ew]
                    #     eye_img_l = cv2.resize(eye_img_l, dsize=IMG_SIZE)
                    #     color = (0,255,0)
                        
                    #     prediction = predict_eye(eye_img_l, model)
                    #     if prediction == 0:
                    #         eye_status='0'
                    #     cv2.rectangle(face,(ex,ey),(ex+ew,ey+eh),color,2)
                    ...
                    #     eye_img_r = gray_face[ey:ey+eh,ex:ex+ew]
                    #     eye_img_r = cv2.resize(eye_img_r, dsize=IMG_SIZE)
                    #     eye_img_r = cv2.flip(eye_img_r, flipCode=1)
                    #     color = (0,255,0)

                    #     prediction = predict_eye(eye_img_r, model)
                    #     if prediction == 0:
                    #         eye_status='0'
                    #     cv2.rectangle(face,(ex,ey),(ex+ew,ey+eh),color,2)
                    # hist += eye_status
                    
                    if isBlinking(hist, 1):
